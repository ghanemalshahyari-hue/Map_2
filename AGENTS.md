...
- memory dir (`MEMORY.md` index) — **why**: locked decisions, feedback, deferred items.
  On a machine without the local memory dir (`~/.Codex/.../memory/` — it's git-ignored and
  machine-local), read the committed mirror at **`docs/session-memory/`** (its `README.md`
  shows how to rehydrate it into `~/.Codex`).
...
- `.Codex/skills/audit-app/` — the refresh procedure.
