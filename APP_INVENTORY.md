...
- **Scenario Authoring Mode — IN PROGRESS (Wave 3):** **Slices 1 + 2A + 2B BUILT & wired** — `shell/scenario-edit-mode.js` (`window.AppEditMode`, mounted in app.html). **Slice 1** = Metadata + Sides + Posture. **Slice 2A (Geography, 2026-06-02)** adds **AO** (`map_bbox` + `ao_boundaries[]` with a "Use map_bbox as AO" one-click) and **Forces Geometry** (`obj` with `carver` 0..60 client-side hard block, `pipeline`, `throughput_ceilings_km` H12/H24/H48/H72/H120, `bls_template[]`). **Slice 2B (Forces, 2026-06-02)** adds a **Forces card** with Red OOB + Blue OOB sub-sections — `red_units[]` add/remove/edit (uid, label, bls=select-from-bls_template, appear, role=select-of-7-CMO-roles, coord, echelon, strength, sidc) and `blue_units_initial[]` add/remove/edit (unit_uid, base_id, coord, echelon, sidc). **Writes into the in-memory scenario draft, NOT the durable `/api/units` SQLite store** (the two stores are unbridged; durable ORBAT is for persistent force structure, not scenario authoring). `blue_units_base_ids` is **derived from `blue_units_initial[].base_id` on Save** (single source of truth, no parallel-list maintenance). **Add Red unit is disabled until at least one BLS exists** with a visible hint; the disable-state is **live-reactive** to BLS add/remove in the Geometry card via a small cross-card refresh hook (no full editor re-render, no input-focus loss). Save now also repaints the map via `AppAdjudicatorMap.drawScenario` + `fitScenarioAO` (picks up new unit markers for free). Combined hard rules mirror `scenario-validator.js` — for 2B: `red_units[].bls` ∈ `bls_template[].name`, `red_units[].appear` ∈ steps range, `uid` / `unit_uid` non-empty + unique. Static tests `test-edit-mode-slice2a.js` (50/50 green) and `test-edit-mode-slice2b.js` (46/46 green) round-trip the playbook sample against the **real** validator. Pure helpers exposed at `AppEditMode._testing`. **Browser-verified end-to-end** via `rmooz-web-verify` (scripts/verify-server.js). **Next = DOC1: Doctrine/ROE/WRA** — the highest-value engine gap per the playbook (Step 5 ⚠️ GAP); makes AI proposals auditable. `[[project_workspace_editable_owner_ruling]]`.
