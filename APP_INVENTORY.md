...
- **Scenario Authoring Mode — IN PROGRESS (Wave 3):** **Slices 1 + 2A BUILT & wired** — `shell/scenario-edit-mode.js` (`window.AppEditMode`, mounted in app.html). **Slice 1** = Metadata + Sides + Posture. **Slice 2A (Geography, 2026-06-02)** adds **AO** (`map_bbox` + `ao_boundaries[]` with a "Use map_bbox as AO" one-click) and **Forces Geometry** (`obj` with `carver` 0..60 client-side hard block, `pipeline`, `throughput_ceilings_km` H12/H24/H48/H72/H120, `bls_template[]`) — all editable on the same in-memory working copy. Save now also repaints the map via `AppAdjudicatorMap.drawScenario` + `fitScenarioAO` (geometry edits must redraw; Slice 1 only refreshed panels). Hard rules mirror `scenario-validator.js`. The P0 schema (`scenario-authoring-schema.js`) remains **consumed** (template/gap-fill/diagnose). Pure helpers exposed at `AppEditMode._testing` for the static test `test-edit-mode-slice2a.js` (50/50 green; final draft validates `ok:true` against the real validator). **Next = Slice 2B: Forces** — `red_units[]` / `blue_units_initial[]` add/edit/remove inside Edit Mode, driving the existing ORBAT-dock + cursor-follow placement plumbing to write `coord` into the draft (not the durable `/api/units` DB). Then Doctrine/ROE, Missions/Events. `[[project_workspace_editable_owner_ruling]]`.
