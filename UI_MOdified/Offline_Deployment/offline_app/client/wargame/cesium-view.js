...
(function () {
    ...
    // Offline deployment: Cesium must be provided as a local asset package.
    // No CDN fallback is attempted.
    const CESIUM_BASE    = '/lib/cesium';
    ...
    function ensureInit() {
        ...
        // Offline imagery: use the same resolver as the 2D map. Never request OSM.
        try {
            const offlineMap = window.__RMOOZ_OFFLINE_MAP__;
            const tileUrl = offlineMap && offlineMap.activeTileUrl;
            if (tileUrl) {
                viewer.imageryLayers.addImageryProvider(
                    new Cesium.UrlTemplateImageryProvider({
                        url:          tileUrl,
                        credit:       'Local Offline Tiles',
                        minimumLevel: 0,
                        maximumLevel: 19,
                    })
                );
        ...
        // Dark military tint — dim the base tiles so the symbols pop
    ...
    async function setVisible(show) {
        ...
        if (!loaded) {
            ...
            cEl.innerHTML = `<div style="color:#9ab;font-size:13px;text-align:center;padding:24px;max-width:360px;">
                ...
                Install the local Cesium package under <code>/lib/cesium</code> to enable 3D offline.<br><br>
        ...
        try { window.dispatchEvent(new CustomEvent('cesium:ready')); } catch (_) {}
