 ...
 *   window.openSelectedObjectPanel(object)
 * ========================================================================== */
(function () {
    'use strict';
    ...
    function setSelectedObjectMeta(panel, kind) {
        if (!panel) return;
        panel.setAttribute('data-selected-object-panel', 'true');
        panel.setAttribute('data-object-kind', kind || 'unknown');
    }
    function openUnitObject(object) {
        var data = object && (object.data || object.unit || object);
        if (!data) return openUnsupportedObject(object, 'Unit selection unavailable');
        if (window.AppUnitStatusPanel && typeof window.AppUnitStatusPanel.populatePanel === 'function') {
            window.AppUnitStatusPanel.populatePanel(data, Date.now());
        ...
        if (typeof document !== 'undefined' && typeof document.dispatchEvent === 'function') {
            var evt;
            try {
                evt = new CustomEvent('rmooz:unit-selected', { detail: { unit: data, selectedAt: Date.now() } });
            } catch (_) {
                try {
                    evt = document.createEvent('CustomEvent');
                    evt.initCustomEvent('rmooz:unit-selected', false, false, { unit: data, selectedAt: Date.now() });
                } catch (__) {}
            }
    ...
    function openInfrastructureObject(object) {
        var panel = ensurePanel();
        if (!panel) return null;
        setSelectedObjectMeta(panel, 'infrastructure');
        panel.innerHTML = '<header class="bsp-header"><div><div class="bsp-title">Selected Object</div>' +
            '<div class="bsp-subtitle">Infrastructure</div></div><button class="bsp-close" type="button" title="Close">x</button></header>' +
            '<section class="bsp-section"><h3>Infrastructure</h3><div class="bsp-empty">Infrastructure status support pending</div></section>';
        panel.removeAttribute('hidden');
        var closeBtn = panel.querySelector('.bsp-close');
        if (closeBtn) closeBtn.addEventListener('click', close);
        return panel;
    }
    function openUnsupportedObject(object, message) {
        var panel = ensurePanel();
        if (!panel) return null;
        var kind = object && object.object_kind ? String(object.object_kind) : 'unknown';
        setSelectedObjectMeta(panel, kind || 'unknown');
        panel.innerHTML = '<header class="bsp-header"><div><div class="bsp-title">Selected Object</div>' +
            '<div class="bsp-subtitle">' + esc(kind || 'unknown') + '</div></div><button class="bsp-close" type="button" title="Close">x</button></header>' +
            '<section class="bsp-section"><h3>Unsupported Object</h3><div class="bsp-empty">' +
            esc(message || 'Unsupported selected object type') + '</div></section>';
        panel.removeAttribute('hidden');
        var closeBtn = panel.querySelector('.bsp-close');
        if (closeBtn) closeBtn.addEventListener('click', close);
        return panel;
    ...
    function render(anchor, payload) {
        var panel = ensurePanel();
        if (!panel) return;
        setSelectedObjectMeta(panel, 'base');
        anchor = anchor || {};
        payload = payload || {};
    ...
    function openSelectedObjectPanel(object) {
        object = object || {};
        var kind = String(object.object_kind || object.kind || '').toLowerCase();
        if (kind === 'unit') return openUnitObject(object);
        if (kind === 'base') {
            var payload = object.payload || (object.data && object.data.payload) || window.__rmoozStep1SelectedObjectPayload || {};
            var anchor = object.data || object.base || object.anchor || {};
            render(anchor, payload);
            var panel = document && document.getElementById(PANEL_ID);
            setSelectedObjectMeta(panel, 'base');
            if (panel) {
                panel.setAttribute('data-source', object.source || '');
                panel.setAttribute('data-review-only', object.review_only === false ? 'false' : 'true');
                panel.setAttribute('data-exact-unit-position', object.exact_unit_position === true ? 'true' : 'false');
            }
            return panel;
        }
        if (kind === 'infrastructure') return openInfrastructureObject(object);
        return openUnsupportedObject(object);
    }
