...
(function () {
    ...
    function render(anchor, payload) {
        ...
        if (orphaned.length) {
            // STEP1-BASE-STATUS-PER-BASE-UNITS-A: wrap in collapsed <details> so the global
            // unassigned list never looks like it belongs to the currently-selected base.
            html += '<section class="bsp-section"><details class="bsp-unassigned-details">' +
                '<summary class="bsp-unassigned-summary" dir="auto">Global Unassigned — not linked to this base (' + orphaned.length + ')' +
                '<span class="bsp-dim" dir="rtl"> — غير مُسندة عامة — ليست مرتبطة بهذه القاعدة</span></summary>' +
                '<div class="bsp-catreq" dir="auto">These proposed units are present in the review payload but did not match any base anchor by id, name, country/side, or coordinates. They are NOT linked to this base. — هذه الوحدات لم تطابق أي مرساة قاعدة. لا تنتمي لهذه القاعدة.</div>' +
                renderUnitTable(orphaned) + '</details></section>';
        }
