...
(function (root) {
    ...
    function buildRunTimeline(instrumentation) {
        ...
        var timeline = [
            ...
            timelineItem('runtime_context', 'Runtime context', text(step.title, step.key || 'Unknown'), step.status || 'pending')
        ];
        if (num(evidence.scenario_turn, 0) > 0 || num(scc.scenario_turn, 0) > 0) {
            timeline.push(timelineItem('runtime_frame', 'Runtime time observed', 'Runtime frame ' + num(evidence.scenario_turn, scc.scenario_turn) + (evidence.current_actor ? ' / ' + evidence.current_actor : ''), 'pass'));
        }
        if (evidence.current_phase_name || scc.current_phase_name) {
            timeline.push(timelineItem('runtime_group', 'Runtime group', text(evidence.current_phase_name, scc.current_phase_name), 'pass'));
        }
    ...
    function renderDebriefHtml(debrief) {
        ...
        var html = '<div class="cmo-wargame-after-action-debrief ' + statusClass('cmo-wargame-after-action-debrief', outcome.status) + '" data-cmo-after-action-debrief="true">' +
            ...
            '<dl class="cmo-wargame-after-action-debrief-meta">' +
                ...
                '<div><dt>Runtime frame / actor</dt><dd>' + esc(num(evidence.scenario_turn, scc.scenario_turn)) + (evidence.current_actor ? ' / ' + esc(evidence.current_actor) : '') + '</dd></div>' +
            ...
            '<div class="cmo-wargame-aar-section"><strong>AAR runtime timeline</strong>' + listHtml(debrief.run_timeline, 'No runtime timeline available.', 'cmo-wargame-aar') + '</div>' +
