...
(function (root) {
    ...
    function buildObserveChecklist(card, scc, flow) {
        ...
        var items = [
            ...
            checklistItem('time_observed', 'Runtime time observed', 'Runtime frame ' + num(scc.scenario_turn, 0), num(scc.scenario_turn, 0) > 0 ? 'pass' : (running ? 'warn' : 'pending')),
    ...
    var DIGEST_LABELS = {
        ...
        scenario_turn: 'Runtime frame',
        current_actor: 'Current actor',
        current_phase_name: 'Runtime group',
    ...
    function summaryText(instrumentation) {
        ...
        var lines = [
            ...
            'Runtime context: ' + (step.title || step.key || 'Unknown'),
            'Runtime frame: ' + num(scc.scenario_turn, 0) + (scc.current_actor ? ' / actor ' + scc.current_actor : ''),
    ...
    function renderRunInstrumentationHtml(instrumentation) {
        ...
        var html = '<div class="cmo-wargame-run-instrumentation" data-cmo-run-instrumentation="true">' +
            ...
            '<dl class="cmo-wargame-run-instrumentation-meta">' +
                ...
                '<div><dt>Runtime context</dt><dd>' + esc(step.title || step.key || 'Unknown') + '</dd></div>' +
                '<div><dt>Runtime frame / actor</dt><dd>' + esc(num(scc.scenario_turn, 0)) + (scc.current_actor ? ' / ' + esc(scc.current_actor) : '') + '</dd></div>' +
                '<div><dt>Runtime group</dt><dd>' + esc(scc.current_phase_name || scc.phase_status || 'Not committed') + '</dd></div>' +
            '</dl>' +
