...
| `free-fight-llm-plan.js` (231 ln) | Free Fight demo AI. Produces advisory attack/reaction plans for the map demo. Has its own `parseJsonResponse` (find-first-`{}`). Enabled only via `RMOOZ_ALLOW_SIM_RUN=1`. | via `ai-provider.js` ✅ | env vars: `RMOOZ_FREE_FIGHT_PROVIDER`, `RMOOZ_FREE_FIGHT_MODEL`, `RMOOZ_AI_PROVIDER` | No (LLM) | Has its OWN `parseJsonResponse` (4th JSON extraction function); uses multi-level env var cascade that partially duplicates `ai-config.js`; no `<think>` stripping |
