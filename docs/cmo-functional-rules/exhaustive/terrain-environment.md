...
- **Edge cases / quirks:** No explicit horizon-distance formula is stated — qualitative only ("be
  ...
  horizon model and the "altitude → horizon distance" calculator, see the radar-horizon rule in
  `1-movement-detection.md` — that detailed model is in the Movement & Detection bucket.)
