...
| **T-3 Analysis products v1** — ✅ **DONE 2026-06-12 (GIS-TERRAIN-1, owner-narrowed: server helpers + tests only)** | `server/terrain-analysis.js`: `analyzePointTerrain` / `analyzeRouteTerrain` (distance/profile/slope + `mobility_warning` none·slow_go·no_go·unknown) / `analyzeAreaTerrain` (8×8 grid, polygon-ring filtering, neighbor-pair slope) / `analyzeLandingZoneSuitability` (flatness + size + optional coastline-layer proximity → suitable·marginal·unsuitable·unknown) + PM attach helpers (`attachRouteTerrain`/`attachLocationTerrain`/`attachLandingZoneTerrain` → `terrain_analysis[]` with `ref_kind`/`ref_id`). All thresholds INTERIM-flagged for G-6; structured unknowns when DEM missing. **Moved out to T-4:** Location-Intelligence candidate wiring (§6), scenario-schema additions (§4), registry/GDAL recipes. | M |
| **T-4A Location-Intelligence wiring** — ✅ **DONE 2026-06-12 (GIS-TERRAIN-1)** | Opt-in `includeTerrain:true` on `resolveMention` / `enrichPlanningModelLocations`: each lat/lon candidate gains advisory `candidate.terrain` (`terrain_available`, `elevation_m`, separate terrain confidence, warnings, `terrain_layer` source, `needs_review`); default OFF = G-3B byte-identical (terrain stack lazy-loads only on use); no-coord candidates skipped; DEM-less ⇒ structured `dem_not_configured`/`no_terrain_data`. Advisory only — never rejects/moves/re-scores a candidate. Test: `test-location-terrain-t4a.js`. **+ T-4A-V (same day): placement panel renders the block** (advisory-only label, unavailable=warning-chip, bilingual; render-only, no browser terrain calls; `test-placement-terrain-t4av.js`). | S |
| **T-4B..D Consumer wiring (remaining)** | COA terrain_warnings + card chips §7; tasking route_terrain §8 (lands WITH G-4); brief-to-scenario LZ check feeding the D6 landing-area confirmation; hillshade tile route + Leaflet overlay | M |
