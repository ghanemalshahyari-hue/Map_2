...
| Phase | Scope | Size |
|---|---|---|
| **T-1 Wire what exists** | Endpoint + Leaflet overlay for the ORPHANED `dem-service.js` (elevation lookup + hillshade tiles behind the existing proxy); fix the machine-local default path (env/`.env.offline`); inventory row + drift note resolved | S |
| **T-2 Registry + service** | `terrain.meta.json` registry, generalize into `terrain-service.js` (multi-AO), GDAL preprocessing recipes documented in `devtools/`, point/profile/region API, AO-keyed cache | M |
