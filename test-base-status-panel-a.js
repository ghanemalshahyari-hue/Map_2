...
var realOpenSelectedObjectPanel = global.window.openSelectedObjectPanel;
...
ok('exposes common selected object panel API', typeof global.window.openSelectedObjectPanel === 'function');
...
global.window.openSelectedObjectPanel({
    ...
    review_only: true,
    exact_unit_position: false,
    data: candidateFromBase(hamedan),
    payload: payload,
});
...
ok('proposed units display in selected object panel', /F-14A Tomcat/.test(panel.innerHTML) && /F-4E Phantom II/.test(panel.innerHTML));
ok('exact_unit_position remains false on common panel', panel && panel.attrs['data-exact-unit-position'] === 'false');
...
global.window.openSelectedObjectPanel = function (obj) {
    selectedObjectCall = obj;
    clicked = obj && obj.object_kind === 'base' && obj.data && obj.data.base_name_en === 'Bandar Abbas';
    BasePanel.open(obj.data, payload);
};
global.window.L = {
...
ok('placement marker calls required base selected-object shape',
    selectedObjectCall &&
    ...
    selectedObjectCall.data && selectedObjectCall.data.base_name_en === 'Bandar Abbas');
...
ok('unknown object_kind does not crash and renders unsupported placeholder', (function () {
    try {
        realOpenSelectedObjectPanel({ object_kind: 'mystery_object', data: { label: 'Odd thing' } });
        return panel && /Unsupported selected object type/.test(panel.innerHTML) && panel.attrs['data-object-kind'] === 'mystery_object';
    } catch (_) {
        return false;
    }
})());
