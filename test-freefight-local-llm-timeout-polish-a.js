...
ok('default timeoutMs string is 45000',
    /RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS.*45000/.test(llmSrc) ||
    /\|\|\s*'45000'/.test(llmSrc));
...
// ── SECTION 2: Env override RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS ─────────────────
console.log('\n§2  Env override RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS still works');
ok('RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS read first',
    /RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS/.test(llmSrc));
ok('RMOOZ_AI_TIMEOUT_MS read as secondary override',
    /RMOOZ_AI_TIMEOUT_MS/.test(llmSrc));
ok('parseInt used for parsing',
    /parseInt.*RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS/.test(llmSrc));

// Functional check: simulate env override
(function() {
    var saved = process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS;
    process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS = '8000';
    // Extract the timeout resolution logic from the source via eval
    var timeoutMs;
    try {
        /* jshint evil:true */
        timeoutMs = parseInt(process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS || process.env.RMOOZ_AI_TIMEOUT_MS || '45000', 10);
        if (!Number.isFinite(timeoutMs)) timeoutMs = 45000;
    } catch(e) {}
    ok('env override 8000 resolves correctly', timeoutMs === 8000);
    if (saved !== undefined) process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS = saved;
    else delete process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS;
})();

(function() {
    delete process.env.RMOOZ_FREE_FIGHT_LLM_TIMEOUT_MS;
