...
(function () {
    ...
    // After the doc-review-persist ↔ json-loss-fix merge, the gate also accepts a
    // Step 1 JSON-understanding import (jsonOnly) as a single-input review path,
    // alongside mdmpOnly; DOCX still requires both Red + Blue (docxReady).
    ok('analyze gate requires docxReady, with mdmpOnly + jsonOnly single-input exceptions',
        /el\.analyze\.disabled = st\.running \|\| !\([^)]*docxReady[^)]*\)/.test(src) && /jsonOnly/.test(src) && /mdmpOnly/.test(src));
